...
class GameRunner:
    ...
    def needs_first_run_setup(self) -> bool:
        """True when the next launch is a slow first run (Proton runtime
        download + prefix creation). Named platform-neutrally because
        main_window uses it on both platforms; Windows always returns False.
        """
        prefix = os.path.expanduser(self.config.wine_prefix)
        return not os.path.exists(os.path.join(prefix, "system.reg"))
    ...
    def _run_createprefix(self, umu: str, env: dict, log_file) -> bool:
        ...
        if rc != 0:
            ...
            log_file.flush()
        return not self.needs_first_run_setup()
